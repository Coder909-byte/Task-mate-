...
function loadTasks() {
  ...
  const savedTime = localStorage.getItem("taskmate_timer_left");
  const savedInitial = localStorage.getItem("taskmate_timer_initial");
  const savedFocusId = localStorage.getItem("taskmate_timer_focus_id");

  if (savedTime) timeLeft = parseInt(savedTime);
  if (savedInitial) initialTime = parseInt(savedInitial);
  if (savedFocusId && savedFocusId !== "null") {
    focusedTaskId = savedFocusId;
  }
}
...
function saveTasks() {
  ...
  localStorage.setItem("taskmate_timer_left", timeLeft);
  localStorage.setItem("taskmate_timer_initial", initialTime);
  localStorage.setItem("taskmate_timer_focus_id", focusedTaskId);
...
function createTaskElement(task) {
  ...
  const doneSteps = task.subtasks ? task.subtasks.filter((s) => s.done).length : 0;
...
function setupFocusListeners() {
  ...
  if (select) {
    select.addEventListener("change", (e) => {
      if (e.target.value) {
        startFocusSession(e.target.value);
        saveTasks(); // Save focused task selection
      }
    });
  }
  ...
  if (changeBtn) {
    changeBtn.addEventListener("click", () => {
      ...
      saveTasks(); // Clear saved focus task
    });
  }

  // Restore focus UI if task was saved previously
  if (focusedTaskId) {
    startFocusSession(focusedTaskId);
  }
}
...
function refreshFocusDropdown() {
  ...
  select.value = currentVal || focusedTaskId || "";
}
...
function startTimer() {
  ...
  document.getElementById("timerToggle").innerHTML = `<i data-lucide="pause"></i> Pause`;
  if (window.lucide) lucide.createIcons();
  ...
  timerInterval = setInterval(() => {
    if (timeLeft > 0) {
      timeLeft--;
      updateTimerUI();
      // Auto-save time every 5 seconds to prevent loss on refresh
      if (timeLeft % 5 === 0) localStorage.setItem("taskmate_timer_left", timeLeft);
    } else {
...
function stopTimer(message) {
  ...
  timerInterval = null;
  document.getElementById("timerToggle").innerHTML = `<i data-lucide="play"></i> Start Focus`;
  if (window.lucide) lucide.createIcons();
  
  // Final save of time when stopped
  localStorage.setItem("taskmate_timer_left", timeLeft);
...
function setupEventListeners() {
  ...
  document.getElementById("addSubtaskBtn").addEventListener("click", () => {
    ...
    div.innerHTML = `<input type="checkbox" disabled><input type="text" class="subtask-text" placeholder="Step..."><button type="button" onclick="this.parentElement.remove()">×</button>`;
...
function renderCharts() {
  ...
  currentChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ label: "Tasks", data: [counts.todo, counts.doing, counts.done], backgroundColor: ["#f4c95d", "#43b39b", "#f28f7b"], borderRadius: 8 }],
    },
    options: {
      responsive: true,
